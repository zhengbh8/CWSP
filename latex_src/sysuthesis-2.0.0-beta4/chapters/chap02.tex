...
通过上述变换机制，原图中步长为2的局部路径结构（如 $u \rightarrow v \rightarrow w$）被无损压缩为线图中的单条有向边。从空间映射的视角来看，线图本质上是对原图局部路径演化空间的一种高阶展开\cite{van1965interchange}。
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \begin{scope}[xshift=0cm]
      ...
      \path (A) edge node[above,draw=none] {$e_1$} (B)
            (B) edge node[above,draw=none] {$e_2$} (C);
      
      \node[draw=none,below=1.5cm of B] {\textbf{原图:} $G$};
    \end{scope}
    
    % Arrow indicating transformation
    \node[draw=none] (arrow) at (5.5, 0) {\Large $\Rightarrow$};
    ...
    \begin{scope}[xshift=7cm]
      ...
      \path (E1) edge node[above,draw=none] {$(e_1,e_2)$} (E2);
      
      \node[draw=none,below=1.5cm of E1,xshift=10mm] {\textbf{线图:} $L(G)$};
    \end{scope}

  \end{tikzpicture}
  \caption{有向图及其线图的构造转化示例}
